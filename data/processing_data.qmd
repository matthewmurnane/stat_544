...
data <- data %>%
  mutate(SCHL = case_when(
    ...
    SCHL == "2" ~ "Less than High School",
    SCHL == "3" ~ "Less than High School",
    SCHL == "4" ~ "Less than High School",
    SCHL == "5" ~ "Less than High School",
    SCHL == "6" ~ "Less than High School",
    SCHL == "7" ~ "Less than High School",
    SCHL == "8" ~ "Less than High School",
    SCHL == "9" ~ "Less than High School",
    SCHL == "10" ~ "Less than High School",
    SCHL == "11" ~ "Less than High School",
    SCHL == "12" ~ "Less than High School",
    SCHL == "13" ~ "Less than High School",
    SCHL == "14" ~ "Less than High School",
    ...
    SCHL == "23" ~ "Post-bacc professional degree",
...
# adjustment for GRNTP
data$ADJHSG <- as.numeric(data$ADJHSG)

# adjustment for PINCP
data$ADJINC <- as.numeric(data$ADJINC)
```
...
data <- data %>%
  rename(age = agep,
         ...
         gross_rent = grntp,
         education = schl,
         income_personal = pincp,
         health_insurance = hicov,
         snap_benefits = fs,
         income_household = hincp)
```

```{r}
data <- data %>%
  mutate(race = ifelse(hisp != "Not Spanish/Hispanic/Latino", "Hispanic", race))

data <- data %>% 
  mutate(race_simp = ifelse(race %in% c("White", "Black", "Asian", "Hispanic"),
                            race,
                            "Other"))
```
